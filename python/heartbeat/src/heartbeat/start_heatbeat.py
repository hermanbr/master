...
def deploy(host):
    run_subprocess(["ssh", "-i", KEY, f"ubuntu@{host}", "rm -rf ~/heartbeat"])
    run_subprocess(
        [
            "scp",
            "-r",
    ...
    run_subprocess(
        [
            ...
            KEY,
            f"ubuntu@{host}",
            "rm -rf ~/heartbeat/.venv ~/heartbeat/dist",
        ]
    )
    run_subprocess(["ssh", "-i", KEY, f"ubuntu@{host}", f"cd heartbeat && {UV} sync"])
...
def main():
    ...
    try_run_subprocess(
        [
            ...
            ssh(
                UE0,
                f"cd heartbeat && {UV} run src/heartbeat/heartbeat_client.py",
            ),
        ]
    )
    try_run_subprocess(
        [
            ...
            ssh(
                UE1,
                f"cd heartbeat && {UV} run src/heartbeat/heartbeat_server.py",
            ),
        ]
    )
